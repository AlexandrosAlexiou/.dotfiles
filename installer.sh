...
function _nvim_from_source() {
    _print i "neovim" ": a superior vim fork focused on extensiblity and usability"
    _ensure_installed git ninja cmake gettext curl
    if ! [ -d neovim ]; then
        _print info "Fetching Neovim"
        git clone --quiet --depth=1 https://github.com/neovim/neovim.git
    fi
    pushd neovim > /dev/null || return \
...
function _nvim_config() {
    ...
    /opt/homebrew/bin/gcp -arsfT "${SCRIPT_DIR}"/nvim "$nvim_config_path"
...
function _wezterm_config() {
    ...
    /opt/homebrew/bin/gln -sv --backup=numbered "${SCRIPT_DIR}/wezterm/wezterm.lua" "$HOME"/.wezterm.lua
}
...
function _install_tinkertool() {
    ...
    # Create the download command interpolating the PHPSESSID that we got from the cookies and download th dmg file.
